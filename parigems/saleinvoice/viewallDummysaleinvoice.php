...
if (isset($_POST['vat'])) {
	  if($_POST['vat']!=''){
	 $vat=" and i.invoicetype='WithVat' and i.status='1'";
	  }
   }
else
{$vat="";}
...
if (isset($_POST['hform'])) {
	  if($_POST['hform']!=''){
	 $hform=" and i.invoicetype='HForm' and i.status='1'";
	  }
   }
else
{$hform="";}
...
$querysale="SELECT b.*,i.* FROM saleinvoice_dummy i,basic_details b where 1 $vat $hform $cash $hongkong and b.userid=i.userid";
...
<section class="main-section">
	<div class="container-fluid crumb_top">
		<ol class="breadcrumb" id="breadcrumb" style="color: black">
	        ...
	        <li class="active">View All Dummy Sale Invoice</li>
	    </ol>
	   	<div id="toolbar">
	    ...
	    <h3 class="text-center">View All Dummy Sale Invoice</h3>
		
		<form action="viewallDummysaleinvoice.php" method="post">
		<?php
		$getcount1="SELECT b.*,i.* FROM saleinvoice_dummy i,basic_details b where b.userid=i.userid and i.status='1' and i.invoicetype='WithVat'";
		$result1=mysqli_query($con,$getcount1);
		$getcount2="SELECT b.*,i.* FROM saleinvoice_dummy i,basic_details b where b.userid=i.userid and i.status='1' and i.invoicetype='HForm'";
		$result2=mysqli_query($con,$getcount2);
		$getcount3="SELECT b.*,i.* FROM saleinvoice_dummy i,basic_details b where b.userid=i.userid and i.status='1' and i.invoicetype='Cash'";
		$result3=mysqli_query($con,$getcount3);
		$getcount4="SELECT b.*,i.* FROM saleinvoice_dummy i,basic_details b where b.userid=i.userid and i.status='1' and i.invoicetype='Hongkong'";
		...
		<span class="">
         <p class="btn btn-default">
            ...
            <label for="cash" class="label label-warning"><input type="checkbox" onclick="this.form.submit();" name="cash" value="Cash" id="cash" style="display: none;">Cash (<b><?php echo mysqli_num_rows($result2);?></b>)</label>
		...
		<thead>
			<tr>
				...
				<th data-field="TotalQty" data-sortable="true">Customer Name</th>
		...
		<tbody>
			<?php
				...
				while($row=mysqli_fetch_assoc($result)){
					...
					if($invoicetype=='WithVat'){$class='danger';}
					else if($invoicetype=='HForm'){$class='success';}
						...
						if($row['invoicetype']=='HForm')
						{
						echo "<a  href='view_sale_invoiceHform.php?id=".encrypt_decrypt('encrypt',$id)."'  class='btn btn-info'>View</a>&nbsp;";	  
						}else{
						echo "<a  href='view_Dsale_invoice.php?id=".encrypt_decrypt('encrypt',$id)."'  class='btn btn-info'>View</a>&nbsp;";
						}
                        if($status=='1')
                        {
...
function deleteinvoice(id)
        {
			var r =  bootbox.confirm("Are you sure want to delete Invoice Number: "+id+"?", function(result) {
    if (result == true) {
   	...
   	http2.onreadystatechange=function()
   	{
   		...
   		if (http2.readyState==4 )
   				 ...
   				 http2.open("GET","deleteDummysaleinvoice.php?id="+id, true);
   				 http2.send(null);
				 }
	 }); 
		}
    ...
    if (result == true) {
   	...
   	http2.onreadystatechange=function()
   	{
   		...
   		if (http2.readyState==4 )
   				 ...
   				 http2.open("GET","activeDummysaleinvoice.php?id="+id, true);
   				 http2.send(null);
				 }
	 }); 
		}
