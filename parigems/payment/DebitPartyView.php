<?php include"../common/header.php";
   ...
   session_start();
   $id = $_GET['id'];
   $getpaymentQuery= "SELECT * FROM debit_voucher  where  receiptno=$id";
   ...
   $pay =mysqli_fetch_assoc($getpaymentResult);
	...
	$banknameqry = mysqli_query($conn,"SELECT * FROM bankaccounts where status=1 and id=".$pay['bankid']);
...
<div class="container hidden-print content crumb_top">
   <ol class="breadcrumb" id="breadcrumb" style="color: black">
      <li><a href="../common/homepage.php">Home</a></li>
      <li><a href="view_debit_party.php">Debit Voucher Details</a></li>
      <li class="active">Debit Voucher</li>
   </ol>
   <div style="float: right" class="form-group">
      <!--<button  class="btn btn-danger delete"  onclick="window.print()">Print</button>-->
   </div>
   <h2 align="center">Debit Voucher</h2>
   <br>
   <div class="col-xs-6">
      <font><label>Paid To : </label><?php  echo $partyname; ?></font>
   </div>
   <div class="col-xs-3">
      <font><label>Date :</label><?php echo date("d/m/Y", strtotime($date));?> </font>
   </div>
   <div class="col-xs-3">
      <font><label>Voucher Number : </label><?php echo $id;?> </font>
   </div>
   <br>
   <br>
   <br>
   <table class="table " style="margin-top:40px;" id="tableProduct">
         ...
         <?php
             ...
             while($row=mysqli_fetch_assoc($getpaymentResult))
            	 {
				  ...
				  $querypurchase="SELECT * FROM purchaseinvoice where  purchase_invoiceid='".$row['invoiceno']."'";
				   $resultpurchase = mysqli_query($con,$querypurchase);
            ...
            echo 	'	
            	...
            	if($paytype=='cheque')
					...
					echo '<td>'.date("d/m/Y", strtotime($row['chequedate'])).'</td>';
							echo '<td>'.$bankname.'</td>';
					 }
    ...
    <?php echo "<b>Rupees&nbsp;:</b>&nbsp;". no_to_words($totalAmount).'Only<br><br>';?>
   <label>Particulars:<?php echo $notes;?></label>
</div>
</div>
</section>
	 ...
	 $getpaymentQuery= "SELECT * FROM debit_voucher  where  receiptno=$id";
         $getpaymentResult = mysqli_query($con,$getpaymentQuery);
          ...
          <div class="col-xs-3" style="float: right;">
						 <font><label>Voucher No: </label><?php echo $id;?> </font>	<br>
      ...
      <div style="height: 200px;border-left:1px solid black;border-right:1px solid black;">
		  <div class="col-xs-6">
                        <font>Paid To: <?php  echo $partyname; ?></font>
                     </div>
		  <br>
