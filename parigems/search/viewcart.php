...
<body>
  <section class="main-section">
    <div class="container-fluid">
      <ol class="breadcrumb" id="breadcrumb">
        ...
        <li><a href="../search/searchdiamond.php">Search Diamond</a></li>
  ...
  else
  {
   $lastavgdiscount='+'.abs((-1)*sprintf("%.2f",$lastavgdiscount1));
	...
	$productByCode ="SELECT d.*,dp.rap,dp.final FROM diamond_master d,diamond_sale dp,add_to_cart i WHERE d.diamond_id=dp.diamond_id and i.diamondid=d.diamond_id and i.userid='$userid' and i.cartstatus='1'";
...
<p class="text-center">Please go to search diamond add items in Cart to move futher</p>
<?php
      ...
      <p class="pull-right">
        <span style="background-color: #ebcccc;">&nbsp;&nbsp;&nbsp;&nbsp;</span>&nbsp;&nbsp;Out Of Stock
      </p>
     ...
     <table class="table table-condensed" id="table" data-height="400" data-toggle="table" data-search="true">
	   <thead style="font-size:8px;padding: 0px;">
           <tr>
             ...
             <!-- <th data-sortable="true">Sr.No.</th>-->
				<th data-sortable="true">View</th>
                 ...
                 <th data-sortable="true" title="Old Discount">Old Dis</th>
				 <th data-sortable="true" title="Per Carat Rate">P/C $</th>
                 <th data-sortable="true" title="Amount">Amt$</th>
                 <th data-sortable="true" title="Depth">Dpth</th> 
                 <th data-sortable="true" title="Table">Tble</th>
        ...
        <tbody  style="font-size:8px;padding: 0px;">
            <?php
              $i=1;
              $certificteqry1="select i.*,d.*,dp.rap,dp.final from add_to_cart i,diamond_master d,diamond_sale dp where d.diamond_id=dp.diamond_id and  i.cartstatus='1' and i.diamondid=d.diamond_id and i.userid='$userid' ";
              ...
              $did=$row['diamond_id'];
                     ...
                     $chevkcart="select * from add_to_cart_temp where diamondid=$did and userid=$userid  and wishstatus='1'";
              ...
              //echo '<td><input style="width:30px;" type="checkbox" name="check[]"  id="'.$did.'" value="'.$did.'" class="case"  /></td>';
               echo '<td>';
					if(mysqli_num_rows($cartresult) > 0)
						    { ?>
							   <input type="checkbox" class="filter case" name="check[]"  id="<?php echo $did; ?>" value="<?php echo $did; ?>" onclick="addtocart_temp('remove','<?php echo $did; ?>','<?php echo $currentraprate;?>')" checked /><br>
					  <?php }
					        else
							{ ?>
								 <input type="checkbox" class="filter case" name="check[]"  id="<?php echo $did; ?>" value="<?php echo $did; ?>" onclick="addtocart_temp('add','<?php echo $did; ?>','<?php echo $currentraprate;?>')" /><br>
					   <?php }
					   echo '</td>';
                  echo "<td>";
				  if($certiimage!=''){
				  ...
				  echo "<a href='".$row['videolink']."' target='_blank' data-container='body' data-toggle='tooltip' title='View Video'><i class='fa fa-video-camera' aria-hidden='true'></i></a>";
				  }
				  echo "</td>";
				  echo "<td class='center' data-container='body' data-toggle='tooltip' title='PG Stock Id'>".$row['referenceno']."</td>";
		...
		<input type="text" style="display: none;" name="rapRate[]"   value="<?php echo $currentraprate;?>"  />
            ...
            <tr>
              <td></td><td></td><td></td><td></td><td></td><td></td><td><?php echo sprintf("%.2f",$finalcarat);?></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td><b>Total</b></td>
	  ...
	  if($i > 1){?>
		...
		<!--<button type="submit" class="btn btn-primary" name="place_order" value="place_order">Place Order</button>-->
		<button type="submit" class="btn btn-danger" name="empty_cart" value="empty_cart">Empty Cart</button>
		</center>
		<?php } ?>
  ...
  document.getElementById('button').click();
	...
	function showconfirmmodal()
		...
		else
		{
           ...
           $.get('viewconfirmmodal.php?did=' + did, function(html){
