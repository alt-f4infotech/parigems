                ...
                foreach($diamondid as $key => $value)
				 {
				     ...
				     $updateqry2="Update diamond_status set diamond_status='UNHOLD',userid='$userid',unholdtime='$currentTime' where diamondid=$fordiamondid";
						  if(mysqli_query($con,$updateqry2))
						 {
						  ...
						  {
							$removetcart="delete from `add_to_cart_hold` where userid='$userid' and diamondid='$fordiamondid' and wishstatus='1'";
							$remcartres=mysqli_query($con,$removetcart);
							
						  $checkDiamondOrder=mysqli_query($con,"select * from invoice_product where diamondid='$fordiamondid' and (deliverystatus is NULL OR deliverystatus='0')");
						  if(mysqli_num_rows($checkDiamondOrder) > 0)
						  {
									   $getDiamondDetails=mysqli_query($con,"select d.referenceno,c.certi_no from diamond_master d,certificate_master c where c.certificateid=d.certificate_id and d.diamond_id='$fordiamondid'");
...
?><body onload="bootbox.alert('Diamond Unholded Successfully.', function() {
            window.location.href='holded.php';
				});"></body>
<?php
   }
   else{?>
   ...
   if($diamondid!='' && $amount!=''){
			...
			$bsnsCount=0;$orderError=0;
			    ...
			    $removetcartHold="delete from `add_to_cart_hold` where userid='$userid' and diamondid='$fordiamondid' and wishstatus='1'";
			    $remcartresHold=mysqli_query($con,$removetcartHold);
