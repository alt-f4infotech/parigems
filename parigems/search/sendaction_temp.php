...
switch($_GET["action"]) {
	case "add":
		if(!empty($_GET["code"])) {
			...
			$checkavail="select * from add_to_cart_temp where userid='$userid' and diamondid='$dimndid' and wishstatus!='0'";
			...
			$insertcart="INSERT INTO `add_to_cart_temp`(`userid`, `diamondid`, `wishstatus`, `timestamp`) VALUES ('$userid','$dimndid','1',NOW())";
			...
			$updaterapvalue="update add_to_cart_temp set rap=".$raprate.", discount='".$newDiscount."', carat=".$itemArray['weight']." where diamondid='$dimndid' and userid='$userid'";
	...
	case "remove":
				$dimndid=$_GET['code'];
				//$removetcart="update `add_to_cart_temp` set wishstatus='0' where userid='$userid' and diamondid='$dimndid' and wishstatus='1'";
				$removetcart="delete from `add_to_cart_temp` where userid='$userid' and diamondid='$dimndid' and wishstatus='1'";
   ...
   $cartitem=0;$carat=0;$rap=0;$avg_price=0;
	$getcartqry="SELECT carat,rap,discount FROM add_to_cart_temp where wishstatus='1' and userid='$userid'";
