...
switch($_GET["action"]) {
	case "add":
		if(!empty($_GET["code"])) {
			...
			$checkavail="select * from add_to_cart_hold where userid='$userid' and diamondid='$dimndid' and wishstatus!='0'";
			...
			$insertcart="INSERT INTO `add_to_cart_hold`(`userid`, `diamondid`, `wishstatus`, `timestamp`) VALUES ('$userid','$dimndid','1',NOW())";
			...
			$updaterapvalue="update add_to_cart_hold set rap=".$raprate.", discount='".$newDiscount."', carat=".$itemArray['weight']." where diamondid='$dimndid' and userid='$userid'";
	...
	case "remove":
				...
				$removetcart="delete from `add_to_cart_hold` where userid='$userid' and diamondid='$dimndid' and wishstatus='1'";
			    $remcartres=mysqli_query($con,$removetcart);
	break;
   
   case "reset":
				//$removetcart="update `add_to_cart_hold` set wishstatus='0' where userid='$userid' and diamondid='$dimndid' and wishstatus='1'";
				$resetcart="delete from `add_to_cart_hold` where userid='$userid' and wishstatus='1'";
   ...
   $cartitem=0;$carat=0;$rap=0;$avg_price=0;
	$getcartqry="SELECT carat,rap,discount FROM add_to_cart_hold where wishstatus='1' and userid='$userid'";
