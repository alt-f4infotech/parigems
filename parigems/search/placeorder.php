...
include '../common/header.php';
   ...
   if($diamondid!='' && $amount!=''){
			...
			$bsnsCount=0;$orderError=0;$stockError=0;
				...
				$removetcart_temp="delete from `add_to_cart_temp` where userid='$userid' and diamondid='$fordiamondid' and wishstatus='1'";
			    $remcartTempres=mysqli_query($con,$removetcart_temp);
				
				 $checkqty="select * from invoice_product where userid='$userid' and diamondid='$fordiamondid' and status='1'";
			    ...
			    $remwishres=mysqli_query($con,$removetwishlist);
				 $removetcart="update `add_to_cart` set wishstatus='0',cartstatus='0' where diamondid='$fordiamondid'";
			    $remcartres=mysqli_query($con,$removetcart);
 ...
 $result3 = mysqli_query($con,$querySales3);
   ...
   if($diamondid!=''){
                foreach($diamondid as $key => $value)
				 {
				 $fordiamondid = $diamondid[$key];
				 $removetcart="update `add_to_cart` set cartstatus='0' where userid='$userid' and diamondid='$fordiamondid'";
			     $remcartres=mysqli_query($con,$removetcart);
				 
				 $removetcart_temp="delete from `add_to_cart_temp` where userid='$userid' and diamondid='$fordiamondid' and wishstatus='1'";
			     $remcartTempres=mysqli_query($con,$removetcart_temp);
				 }
?><body onload="bootbox.alert('Diamond Removed From Cart Successfully.', function() {
            window.location.href='viewcart.php';
				});"></body>
<?php
   }
