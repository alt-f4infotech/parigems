...
//   {$vatqry="";}
   ...
   {$deleted="";}
//}
$querypurchase="SELECT p.empid as employeeid,p.*,pt.*,l.* FROM purchaseinvoice p, party pt,location_master l where 1 $vatqry $locaqry $paertqry $ptype $wvat $vat $hform $regular $deleted and p.partyid=pt.partyid and p.locationid=l.locationid";
...
<section class="main-section">
	<div class="container-fluid crumb_top">
		<ol class="breadcrumb" id="breadcrumb" style="color: black">
	        ...
	        <li class="active">View All Purchase Invoice</li>
	    </ol>
	   	<div id="toolbar">
	    ...
	    <h3 class="text-left">View All Purchase Invoice</h3>
	    <hr>
		<form action="viewallpurchase.php" method="post">
			<div class="row">
				<div class="col-sm-3">
				...
				<div class="col-sm-4"><br>
					<div class="form-group">
						...
						<a class="btn btn-warning"  onClick="window.location.href='viewallpurchase.php';">RESET ALL</a>
					</div>
				</div>
			</div>
		
		<?php
		$getcount1="SELECT p.*,pt.*,l.* FROM purchaseinvoice p, party pt,location_master l where p.partyid=pt.partyid and p.locationid=l.locationid and p.ptype='wvat' and p.purchasestatus='1'";
		$result1=mysqli_query($con,$getcount1);
		$getcount2="SELECT p.*,pt.*,l.* FROM purchaseinvoice p, party pt,location_master l where p.partyid=pt.partyid and p.locationid=l.locationid and p.ptype='vat' and p.purchasestatus='1'";
		$result2=mysqli_query($con,$getcount2);
		$getcount3="SELECT p.*,pt.*,l.* FROM purchaseinvoice p, party pt,location_master l where p.partyid=pt.partyid and p.locationid=l.locationid and p.ptype='hform' and p.purchasestatus='1'";
		$result3=mysqli_query($con,$getcount3);
		$getcount4="SELECT p.*,pt.*,l.* FROM purchaseinvoice p, party pt,location_master l where p.partyid=pt.partyid and p.locationid=l.locationid and p.ptype='regular' and p.purchasestatus='1'";
		$result4=mysqli_query($con,$getcount4);
		$getcount5="SELECT p.*,pt.*,l.* FROM purchaseinvoice p, party pt,location_master l where p.partyid=pt.partyid and p.locationid=l.locationid and p.purchasestatus='0'";
			...
			//echo $querypurchase;
				...
				while($row=mysqli_fetch_assoc($result)){
					...
					echo "<tr class='$class'>";
						...
						echo "<a  href='view_purchaseinvoice.php?invoiceno=$encrypted_txt'  class='btn btn-info'>View</a>";
						echo "</td>";
					echo "</tr>";
				}
			?>
