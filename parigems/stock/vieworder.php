  ...
  <section class="main-section">
	<div class="container-fluid crumb_top">
         ...
         <p class="btn btn-default">
            ...
            <label><span  style="background-color: #f0ad4e;border: 1px solid gray;">&nbsp;&nbsp;&nbsp;&nbsp;</span>&nbsp;&nbsp;Business Process</label>
		 </p>
		</span>
			  <div id="toolbar">
	  ...
	  <table class="table table-striped" id="table" data-height="400" data-show-columns="true"
	    ...
	    <tbody>
	      <?php
	        $queryStock="SELECT sum(i.amount) as total,sum(i.qty) as qty,sum(d.weight) as carat,d.*,l.*,i.invoiceid,i.empid,i.deliverystatus FROM diamond_master d,invoice_product i,login l where i.diamondid=d.diamond_id and l.userid=i.userid and  i.pstatus='2' group by i.invoiceid order by i.invoiceid DESC";
	        ...
	        $i=1;
	         while($row=mysqli_fetch_assoc($result))
	         {
				...
				$lastQty=$lastQty+$row['qty'];
				$lastCarat=$lastCarat+sprintf("%.2f",$caret);
				$lastTotal=$lastTotal+sprintf("%.2f",$row['total']);
				 ...
				 if($row['qty']==1)
			  {
				...
				else if($diamond_user_status=='HOLD')
			  {
				$class="warning-hold";
			  }
	        ...
	        ?>
		<tr>
		  <td></td><td></td><td>Total</td><td><?php echo $lastQty;?></td><td><?php echo $lastCarat;?></td><td><?php echo $lastTotal;?></td>
		</tr>
	    </tbody>
	  </table>
	  <?php }
