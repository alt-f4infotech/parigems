...
function addrow(){
	...
	html += '<td><input type="text" name="pcs[]" id="pcs_'+i+'" value="1" class="form-control" required onkeypress="return IsNumeric(event);" ></td>';
	...
	html += '<td><input type="text" name="amount[]" id="amountt_'+i+'" class="form-control d3 totalgrossadd"  required onkeypress="return IsNumeric(event);"></td>';
	html += '</tr>';
	$('table').append(html);
...
function showAjaxModal(uid){
			   var certino=document.getElementById('certi_'+uid).value;
		 		$.get('getcertiidregular1.php?id=' + uid+'&certino='+certino, function(html){
...
$('#total').val((Math.round(grossamt)).toFixed(2));
